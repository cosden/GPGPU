...
__global__ void cdp_simple_quicksort( unsigned int *data, int left, int right, int depth )
{
  ...
  // move the pointers to the right locations
  unsigned int *lptr = data+right;
  unsigned int *rptr = data+left;
  ...
  // compute the new right and left poistions
  ...
  if(left < (rptr-data)) 
  {
	...
	// launch quicksort kernel for new boundaries and depth+1 using 1 thread and 1 block
...
void run_qsort(unsigned int *data, unsigned int nitems)
{
  ...
  // launch quicksort kernel with one thread and one block
...
int main(int argc, char **argv)
{
  ...
  // transfer data
  checkCudaErrors(cudaMemcpy(h_data,d_data,num_items*sizeof(unsigned int),cudaMemcpyHostToDevice));
