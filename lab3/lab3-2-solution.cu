...
int main(int argc, char* argv[])
{
  ...
  calcDyadicGPU<<<dimGrid, dimBlock>>>(dim, d_vecA, d_vecB, d_product);
  
  CUDA_CHECK_RETURN( cudaThreadSynchronize());	// Wait for the GPU launched work 
  CUDA_CHECK_RETURN( cudaGetLastError());
  
  // Copy data from device to host
  CUDA_CHECK_RETURN( cudaMemcpy(h_product, d_product, matBytes, cudaMemcpyDeviceToHost) );
  ...
  CUDA_CHECK_RETURN( cudaFree(d_vecA) );
  CUDA_CHECK_RETURN( cudaFree(d_vecB) );
  CUDA_CHECK_RETURN( cudaFree(d_product) );
